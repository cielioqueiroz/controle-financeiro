...
Arquivo-morto do [`ESTADO-ATUAL.md`](./ESTADO-ATUAL.md). Nada aqui foi editado:
são as rodadas anteriores, na ordem em que estavam, movidas em 2026-09-06
quando o documento de retomada passou de 2.693 linhas.

A regra: **as três rodadas mais recentes ficam no `ESTADO-ATUAL.md`**, o resto
vem para cá. O que descreve o estado do projeto (retomada, fila do que falta,
onde o código está) nunca vem — aquilo não é história, é o presente.

Isto continua valendo a pena ler quando a pergunta é *por que* algo é como é. É
daqui que saíram os ADRs e metade das armadilhas do `AGENTS.md`.
